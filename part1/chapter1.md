...
이 책은 지난 15년 동안 이루어진, 오믹스(omics) 기술을 이용한 인간 뇌 발달 연구의 주요 흐름을 따라간다. 오믹스란 유전체(genomics), 전사체(transcriptomics), 단백질체(proteomics) 등 생물학적 분자를 대규모로 총망라하여 분석하는 접근법의 총칭이다. 이야기는 2011년 예일 대학교의 네나드 세스탄(Nenad Sestan) 연구실이 16개 뇌 영역의 시공간 전사체를 처음으로 체계적으로 매핑한 Kang et al. 논문에서 시작된다. 이 연구에서 인간 유전자의 86%가 뇌에서 발현되며, 그 중 90%가 시간과 공간에 따라 차별적으로 조절된다는 것이 처음으로 체계적으로 밝혀졌다. 이어서 앨런 뇌과학연구소(Allen Institute for Brain Science)의 제러미 밀러(Jeremy Miller)와 동료들이 주도한 BrainSpan 프로젝트는 레이저 미세절제(laser microdissection)를 사용하여 태아 뇌의 각 피질층을 개별적으로 분리하고, 각 층에서 발현되는 유전자의 고해상도 지도를 제공했다. 이 두 연구가 벌크 전사체 시대의 기둥이다.

Part 2에서는 단일 세포 기술이 열어준 새로운 세계로 넘어간다. BICCN(Brain Initiative Cell Census Network)이 생산한 성인 뇌 전체의 세포 아틀라스, 임신 초기 태아 뇌의 발달 아틀라스, 그리고 후성유전체(epigenome)와 3차원 유전체 구조까지 통합한 멀티오믹스 연구들을 다룬다. 후성유전체란 DNA 서열 자체는 바뀌지 않으면서 유전자 발현을 조절하는 화학적 변형들의 총체를 말하는데, 같은 악보라도 연주자가 어떻게 해석하느냐에 따라 전혀 다른 곡이 되는 것에 비유할 수 있다. 이 연구들은 뇌에 3,000가지가 넘는 세포 유형이 존재한다는 것을 밝혀냈고, 크로마틴의 접근성이 유전자의 전사에 선행한다는 발달의 후성유전학적 원리를 단일 세포 수준에서 증명했다. 크로마틴 접근성이 '먼저' 열린다는 것은, 세포가 어떤 유전자를 켜기로 결정하기도 전에 그 유전자 주변의 DNA가 풀려서 준비 상태에 들어간다는 뜻이다. 마치 요리하기로 마음먹기 전에 이미 냉장고가 열려 있는 것처럼.

Part 3에서는 사후 조직의 한계를 넘어서는 도구, 즉 뇌 오가노이드와 어셈블로이드를 다룬다. 오가노이드(organoid)란 인간 줄기세포를 실험실에서 3차원으로 배양하여 실제 장기와 유사한 구조를 만들어낸 조직 덩어리를 말한다. 2013년 랭커스터(Lancaster)와 크노블리히(Knoblich)가 인간 줄기세포로부터 3차원 뇌 유사 조직을 처음 만든 이후, 이 기술은 특정 뇌 영역을 모방하는 영역 특이적 오가노이드로, 그리고 여러 영역의 오가노이드를 물리적으로 융합시켜 신경 회로를 재현하는 어셈블로이드로 진화해왔다. 이 기술은 인간 뇌의 발달 과정을 실시간으로, 그리고 실험적으로 교란하면서 관찰할 수 있는 기회를 제공한다. 동시에 in vitro 모델이 실제 뇌를 얼마나 충실하게 재현하는지에 대한 엄밀한 검증도 요구한다.

요약하면, 이 책은 하나의 단순한 질문에서 출발한다. 인간의 뇌는 어떻게 만들어지는가? 이 질문에 답하기 위해, 벌크 전사체에서 단일 세포로, 단일 세포에서 멀티오믹스로, 그리고 사후 조직에서 살아 있는 오가노이드로 기술이 진화해온 과정을 따라간다. 각 장은 그 분야를 정의한 핵심 논문을 중심으로 구성되며, 그 논문이 어떤 질문에서 출발했고, 어떤 기술을 사용했으며, 무엇을 발견했고, 그 발견이 후속 연구에 어떤 영향을 미쳤는지를 서술한다.
